...
function Root({dispatch, dataSource}) {
  let top = (<div>
    <Button type='primary' icon="plus" onClick={()=>dispatch({type: 'openRoleModal', params :{isNew: true, editData: {}}})}>新增角色</Button>
  </div>);
    const columns = [
      ...
      }},
      {
        title: '操作', width: 200, render: function (text, record, index) {
          ...
          return (<span>
            <Tooltip title="修改角色">
              <Button onClick={e=>dispatch({type: 'openRoleModal', params: {isNew: false, editData: record}})} size={'small'} type="ghost" shape="circle"
                      icon="edit" disabled={disabled} style={{marginRight:5}}/>
            </Tooltip>
            <Tooltip title="删除角色">
              <Button onClick={e=>dispatch({type: 'deleteRole', params: {roleId: record.id}})} size={'small'} type="ghost" shape="circle"
                    icon="close" disabled={disabled} style={{marginRight:5}}/>
            </Tooltip>
            <Tooltip title="设置功能模块">
              <Button onClick={e=>dispatch({type: 'openModuleModal', params: {roleId:  record.id, isSet: true}})} size={'small'} type="ghost" shape="circle"
                    icon="setting" disabled={disabled} style={{marginRight:5}}/>
            </Tooltip>
            <Tooltip title="查看功能模块">
              <Button onClick={e=>dispatch({type: 'openModuleModal', params: {roleId:  record.id, isSet: false}})} size={'small'} type="ghost" shape="circle"
                    icon="bars"/>
            </Tooltip>
          </span>);
        }.bind(this)
