  ...
  render() {
    ...
    const loop = function(data) {
      ...
      return data.map((item) => {
        if (item.children && item.children.length > 0) {
          ...
          return <SubMenu key={item.id} className="leftMenu-background-color" title={<span>{item.icon == "" ? "": <Icon type={item.icon} />}{item.name}</span>}>{loop(item.children)}</SubMenu>;
        }
        ...
        return <Menu.Item  className="leftMenu-background-color" key={item.id}><span>{item.icon == "" ? "": <Icon type={item.icon} />}{item.name}</span></Menu.Item>;
      });
    }
    ...
    return (
      <Layout className="tabRoot">
        <Header style={{height: 60, paddingLeft:10, paddingRight:15, background: 'url(upm/main/tab/img/nav.png) repeat-x right top'}}>
          <div style={{float: 'left', marginRight:5, background: 'url(upm/main/tab/img/nav_title.png) top center no-repeat'}}>
          ...
          <Icon
            ...
            onClick={this.toggle}
          />
          <div style={{color:'white', float:'right', paddingTop:5, paddingRight:15,height:57,lineHeight:'57px'}}>
            <span style={{marginRight:5}}>{userInfo.userName == null ? '' : userInfo.userName}</span>
            <Tooltip title="修改密码">
              <a href="#" onClick={()=>this.setState({modifyPwVisible:true})}>
                <Icon type="setting" style={{marginLeft:2, fontSize:18,color:'white',verticalAlign:'middle',marginBottom:3, marginRight:3}}/>
              </a>
            </Tooltip>

            <Tooltip title="退出系统">
              <a href="#" onClick={MainAction.logout.bind(this)}>
                <Icon type="logout" style={{fontSize:18, marginLeft:2, color:'white',verticalAlign:'middle',marginBottom:3}}/>
              </a>
            </Tooltip>
            <ModifyPassword  visible={this.state.modifyPwVisible} modifyPwOk={this.modifyPwOk.bind(this)} modifyPwCancel={this.modifyPwCancel.bind(this)}></ModifyPassword>
          </div>
          <div style={{float:'right',marginTop: 10}}>
            <Icon type="user" className="img"/>
          </div>
          {this.state.isTopMenu == true ? <Menu onClick={this.handleMenuClick.bind(this)} style={{marginTop:10, marginRight:20, float:'right', border:0, background:'none'}}
        ...
        <Layout>
          <Sider collapsible trigger={null} collapsedWidth={0}
                 collapsed={this.state.collapsed} className="overflow leftMenu-background-color" style={{width:210, borderRightWidth:2,  borderRight:'1px solid #bfbfbf'}}>
            <Menu  onClick={this.handleModuleClick.bind(this)} style={{ width: 210, backgroundColor: '#e9e9e9' }} defaultOpenKeys={this.state.defaultOpenKeys} defaultSelectedKeys={this.state.defaultSelectedKeys} mode={this.state.mode}>
              {moduleTree}
            </Menu>
          </Sider>
