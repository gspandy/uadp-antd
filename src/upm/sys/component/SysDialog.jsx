	...
	return (<Modal title={title} width={620} okText="保存" cancelText="返回" visible={visible} confirmLoading={loading}
		...
		<Form>
			...
			<Card title="模拟登陆" bordered={false} extra={
				getFieldDecorator('isCheckedLogin', {initialValue: false})(<Switch
				...
				<FormItem {...formItemLayout} label="登陆地址">
					{getFieldDecorator('loginUrl',
						{initialValue: editData.loginUrl})(<Input autoComplete="off"
																  disabled={!editData.isCheckedLogin}></Input>)}
				</FormItem>
				<FormItem {...formItemLayout} label="用户名">
					{getFieldDecorator('loginUsername',
						{initialValue: editData.loginUsername})(<Input autoComplete="off"
																	   disabled={!editData.isCheckedLogin}/>)}
				</FormItem>
				<FormItem {...formItemLayout} label="密码">
					{getFieldDecorator('loginPassword',
						{initialValue: editData.loginUsername})(<Input type="password" autoComplete="off"
																	   disabled={!editData.isCheckedLogin}/>)}
				</FormItem>
			</Card>
		</Form>
