...
function UserModal({dispatch, form, userType, visible, loading, isNew, editUser, currentUserType}) {
	...
	return (<Modal title={title} okText="保存" cancelText="返回" visible={visible} width={700}
				...
				<Col span={12}>
					<FormItem {...formItemLayout} label="用户姓名">
						{getFieldDecorator('name', {
							...
							<Input autoComplete="off" onChange={e => dispatch({
								type: 'generatePinyin',
								scope: 'self',
								...
								<Select>
									{userType.map(function (item, i) {
										...
										return <Select.Option key={item.code}
															  value={item.code}>{item.value}</Select.Option>
									})}
								</Select>
							)}
						</FormItem>}
			...
			<Row>
				<Col span={24}>
					<FormItem {...oneItemLayout} label="描述">
						{getFieldDecorator('remark', {initialValue: editUser.remark})(<Input type="textarea" autosize
																							 autoComplete="off"/>)}
					</FormItem>
				</Col>
			</Row>
